...
test("contact remains a native POST and legal anchors survive static rendering", () => {
  ...
  assert.match(contact, /name="_next" value="https:\/\/preview\.rangeway\.co\/contact\/thanks\/"/);
...
test("built site keeps the shared header fixed and anchor-safe", () => {
  ...
  assert.match(css, /--sun:#f4a855/);
  assert.match(css, /\.home-hero__story\{[^}]*background:var\(--sun\);[^}]*color:var\(--navy-deep\)/);
  assert.match(css, /\.home-hero__headline\{[^}]*color:var\(--navy-deep\)/);
